...
# 관리자 계정 생성
@admin_router.post("/singup", response_model=Admin)
def create_admin(admin: Admin, db: Session = Depends(get_session), user_role: str = Depends(get_current_user_role)):
    Permission.check_permission(user_role, [Role.admin])
